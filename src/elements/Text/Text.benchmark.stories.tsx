...
const BenchmarkText = ({
  ...
  const onRender: ProfilerOnRenderCallback = (id, phase, actualDuration) => {
    if (phase === "mount") {
      ...
      console.log(
        `📝 [BEFORE RESTYLE] Text ${count} components (${variant}) - Mount: ${actualDuration.toFixed(
          2
        )}ms`
      )
      ...
      console.log(
        `📝 [BEFORE RESTYLE] Text ${count} components (${variant}) - Update: ${actualDuration.toFixed(
          2
        )}ms`
      )
    }
  ...
  return (
    <Box p={2} backgroundColor="mono5">
      ...
      <Profiler id={`text-benchmark-${count}-${variant}`} onRender={onRender}>
...
export default {
  title: "Performance/Text Benchmark (Before Restyle)",
  component: Text,
}
    ...
    <ScrollView>
      <Box p={2} gap={2}>
        <Text variant="xl" weight="medium" mb={2}>
          Text Component Performance Baseline
        </Text>
        <Text variant="sm" color="mono60" mb={4}>
          Testing with styled-components before Restyle migration
        </Text>
      ...
      <Box p={2}>
        ...
        {variants.map((variant) => (
          <Box key={variant} mb={2}>
            ...
            <Profiler id={`variant-${variant}`} onRender={createOnRender(variant)}>
              <Box flexWrap="wrap" flexDirection="row" gap={0.5}>
    ...
    <ScrollView>
      <Box p={2}>
        ...
        <Box mb={4}>
          ...
          <Profiler id="simple-text-props" onRender={onRenderSimple}>
            <Box flexWrap="wrap" flexDirection="row" gap={0.5}>
              {Array.from({ length: 100 }).map((_, i) => (
        ...
        <Box>
          ...
          <Profiler id="complex-text-props" onRender={onRenderComplex}>
            <Box flexWrap="wrap" flexDirection="row" gap={0.5}>
              {Array.from({ length: 100 }).map((_, i) => (
                <Text
