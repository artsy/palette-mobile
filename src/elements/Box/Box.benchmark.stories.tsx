...
const BenchmarkBox = ({ count }: { count: number }) => {
  ...
  const onRender: ProfilerOnRenderCallback = (id, phase, actualDuration) => {
    if (phase === "mount") {
      ...
      console.log(
        `📦 [BEFORE RESTYLE] Box ${count} components - Mount: ${actualDuration.toFixed(2)}ms`
      )
      ...
      console.log(
        `📦 [BEFORE RESTYLE] Box ${count} components - Update: ${actualDuration.toFixed(2)}ms`
      )
    }
  }
  ...
  return (
    <Box p={2} backgroundColor="mono5">
      ...
      <Profiler id={`box-benchmark-${count}`} onRender={onRender}>
...
export default {
  title: "Performance/Box Benchmark (Before Restyle)",
  component: Box,
}
    ...
    <ScrollView>
      <Box p={2} gap={2}>
        <Text variant="xl" weight="medium" mb={2}>
          Box Component Performance Baseline
        </Text>
    ...
    <ScrollView>
      <Box p={2}>
        ...
        <Box mb={4}>
          ...
          <Profiler id="simple-props" onRender={onRenderSimple}>
            <Box flexWrap="wrap" flexDirection="row" gap={0.5}>
        ...
        <Box>
          ...
          <Profiler id="complex-props" onRender={onRenderComplex}>
            <Box flexWrap="wrap" flexDirection="row" gap={0.5}>
              {Array.from({ length: 100 }).map((_, i) => (
                <Box
